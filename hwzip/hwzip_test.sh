...
dir="$PWD"
ver=$(grep 'define VERSION' version.h | grep -Eo '[0-9.]+')
...
HWZIP $ver -- A simple ZIP program from https://www.hanshq.net/zip.html
...
diff -su - <("$dir/hwzip" create -c "Hello, world!" test3.zip foo bar) <<END
...
# Empty with comment.
diff -su - <("$dir/hwzip" create -c "Hello, world!" empty2.zip) <<END
...
diff -su - <("$dir/hwzip" create -m shrink shrink.zip zeros) <<END
...
# Reduce.
diff -su - <("$dir/hwzip" create -m reduce reduce.zip zeros) <<END
...
diff -su - <("$dir/hwzip" create -m implode -c comment implode.zip zeros) <<END
